...
{
    public string HandlerExpression { get; private set; } = null!;

    public object[] Values { get; private set; } = null!;

    public string HandlerExpressionDebugView { get; private set; } = null!;

    public Exception LastException { get; protected set; } = null!;
    public int ExceptionCount { get; protected set; }

    protected Delegate _handler = null!;
    protected string[] _parameters = null!;
    protected DataContainer[] _dataContainers = null!;
...
internal class QuickEventHandler<T1, T2> : QuickEventHandler
{
    ...
    public void Handle(T1 sender, T2 args)
    {
        ...
        try
        {
            _handler.DynamicInvoke(_parArray);
        }
        catch (Exception e)
        {
            LastException = e;
        ...
        finally
        {
            ...
            {
                foreach (DataContainer container in _dataContainers)
                {
    ...
    private bool SetupParameters(object sender, object args)
    {
        ...
        for (int i = 0; i <= 4; ++i)
        {
            ...
            if (i == 0)
            {
                _parArray[_pIndex[i]] = QuickEventExtension.GetP0((sender as DependencyObject)!);
            }
            else if (i == 1)
            {
                _parArray[_pIndex[i]] = QuickEventExtension.GetP1((sender as DependencyObject)!);
            }
            else if (i == 2)
            {
                _parArray[_pIndex[i]] = QuickEventExtension.GetP2((sender as DependencyObject)!);
            }
            else if (i == 3)
            {
                _parArray[_pIndex[i]] = QuickEventExtension.GetP3((sender as DependencyObject)!);
            }
            else if (i == 4)
            {
                _parArray[_pIndex[i]] = QuickEventExtension.GetP4((sender as DependencyObject)!);
            }
        }
        if (failMessage != null)
        {
