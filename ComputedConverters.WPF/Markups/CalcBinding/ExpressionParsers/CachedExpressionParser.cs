...
public sealed class CachedExpressionParser : IExpressionParser
{
    ...
    {
        ExpressionKey expressionKey = new ExpressionKey(expressionText, parameters);

        Lambda cachedLambda = FindInCache(expressionKey);
        if (cachedLambda != null)
            return cachedLambda;

        Lambda lambda = _innerParser.Parse(expressionText, parameters);
    ...
    private Lambda FindInCache(ExpressionKey expressionKey)
    {
        ...
        {
            WeakReference expressionRef = _cachedExpressions[expressionKey];
    ...
    {
        foreach (ExpressionKey key in _cachedExpressions.Keys.ToList())
        {
