...
using ComputedConverters.CalcBinding.Inversion;
using ComputedConverters.CalcBinding.Trace;
...
public class CalcConverter : IValueConverter, IMultiValueConverter
{
    ...
    public object ConvertBack(object value, Type targetType, object parameter, CultureInfo culture)
    {
        ...
        if (_compiledInversedExpression == null)
        {
            try
            {
                Type resType = _compiledExpression.Expression.Type;
                System.Linq.Expressions.ParameterExpression param = Expression.Parameter(resType, "Path");
        ...
        if (_compiledInversedExpression != null)
        {
            try
            {
                ...
                object source = _compiledInversedExpression.Invoke(value);
                return source;
            }
            catch (Exception e)
            {
    ...
    {
        object res = System.Convert.ChangeType(value, type, CultureInfo.InvariantCulture);
        return res;
    }
    ...
    public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
    {
        ...
        {
            Type[] currentValuesTypes = GetTypes(values);
        ...
        try
        {
            object result = _compiledExpression.Invoke(values);
    ...
    private Lambda CompileExpression(object[] values, string expressionTemplate, bool convertBack = false, List<Type> targetTypes = null)
    {
        try
        {
            ...
            if (needCompile)
            {
                List<Type> argumentsTypes = convertBack ? targetTypes : _sourceValuesTypes.Select(t => t ?? typeof(object)).ToList();
    ...
    {
        List<Parameter> parametersDefinition = new List<Parameter>();

        for (int i = 0; i < argumentsTypes.Count(); i++)
        {
            string paramName = GetVariableName(i);
        ...
        Lambda compiledExpression = _parser.Parse(expressionTemplate, parametersDefinition.ToArray());
