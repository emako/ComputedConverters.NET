...
public class PropertyPathAnalyzer
{
    ...
    public List<PathToken> GetPathes(string normPath, IXamlTypeResolver typeResolver)
    {
        ...
        List<Chunk> chunks = GetChunks(normPath);
        List<PathToken> pathes = GetPathes(chunks);
    ...
    private List<Chunk> GetChunks(string str)
    {
        ...
        do
        {
            char c = position >= str.Length ? (char)0 : str[position];
            ...
            {
                bool isDelim = UnknownDelimiters.Contains(c) || QuoteTerminals.Contains(c) || c == 0;
    ...
    private bool GetPath(Chunk chunk, out PathToken pathToken)
    {
        ...
        if (colonPos > 0)
        {
            string left = SubStr(str, 0, colonPos - 1);
    ...
    {
        string[] properties = str.Split(new[] { '.' }, StringSplitOptions.None);
    ...
    private PathToken GetEnumOrStaticProperty(Chunk chunk, string @namespace, List<string> identifierChain)
    {
        PathToken pathToken = null;
        Type enumType;
        string className = identifierChain[0];
        string fullClassName = string.Format("{0}:{1}", @namespace, className);

        List<string> propertyChain = identifierChain.Skip(1).ToList();
        if (propertyChain.Count == 1 && ((enumType = TakeEnum(fullClassName)) != null))
        {
            // enum output
            string enumMember = propertyChain.Single();
    ...
    {
        Type @type = _typeResolver.Resolve(fullTypeName);
